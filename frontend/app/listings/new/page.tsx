...
export default function NewListingPage() {
  ...
  // جلب أصوات My Voices من ElevenLabs — تعكس ما في حسابك + العينات (معاينة)
  useEffect(() => {
    if (!user) return;
    setElevenlabsVoicesLoading(true);
    setElevenlabsVoicesError(null);
    fetch(`${API_URL}/api/ai/user/elevenlabs-voices`, { credentials: "include", headers: getAuthHeaders() })
      ...
      .then(({ ok, status, data }) => {
        if (ok && Array.isArray(data.voices)) {
          ...
          setElevenlabsVoices([]);
          const msg = data?.error || data?.message || (status === 404 ? "المسار غير موجود — حدّث الخادم (Redeploy) للـ Backend." : "تعذر جلب أصوات ElevenLabs.");
          setElevenlabsVoicesError(msg);
        }
      })
      .catch((err) => {
        setElevenlabsVoices([]);
        setElevenlabsVoicesError("تعذر الاتصال بالخادم. تحقق من أن الـ Backend يعمل ومحدّث.");
      })
  ...
  async function handleGenerateVideo() {
    ...
    if (imagesToUse.length < 2) {
      setVideoError("يحتاج الفيديو صورتين على الأقل. اختر 2–8 صور ثم ولد.");
      toast.error("صورة واحدة لا تكفي — اختر صورتين على الأقل", { duration: 4000 });
      return;
    }
                        ...
                        {imagePreviews.map((preview, idx) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {/* Video selection checkbox - يظهر من صورة واحدة فما فوق (aiSupportLevel >= 2) */}
                              {showVideoSelection && (
                                <motion.button
                                  type="button"
                                  onClick={(e) => {
                    ...
                    <div className="space-y-4 mb-4">
                      ...
                      {videoQuality === "full" && (
                        <div>
                          ...
                          <audio ref={voicePreviewAudioRef} className="hidden" onEnded={() => setVoicePreviewPlaying(false)} onError={() => setVoicePreviewPlaying(false)} />

                          {elevenlabsVoicesLoading && (
                            <div className="flex items-center gap-2 py-3">
                              <div className="w-4 h-4 border-2 border-[#D4AF37] border-t-transparent rounded-full animate-spin" />
                              <span className="text-xs text-[#002845]/60">جاري تحميل الأصوات...</span>
                            </div>
                          )}
                          ...
                          {!elevenlabsVoicesLoading && elevenlabsVoices.length > 0 && (
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-3">
                              {elevenlabsVoices.map((v) => {
                                ...
                                const isPlaying = isSelected && voicePreviewPlaying;
                                      ...
                                      {v.previewUrl && (
                                        <button
                                          ...
                                          onClick={(e) => {
                                            ...
                                            voicePreviewAudioRef.current.src = v.previewUrl!;
                                            setVideoVoice(v.id);
                                            voicePreviewAudioRef.current.play().then(() => setVoicePreviewPlaying(true)).catch(() => setVoicePreviewPlaying(false));
                                          }}
                                        ...
                                        >
                                          {isPlaying ? "⏸" : "▶"} {isPlaying ? "إيقاف" : "استمع"}
                                        </button>
                                      )}
                                    </div>
                                  </div>
                          ...
                          {!elevenlabsVoicesLoading && elevenlabsVoices.length === 0 && (
                            <div className="mb-3 p-3 rounded-lg bg-amber-50 border border-amber-200">
                              <span className="text-xs text-amber-700">
                                {elevenlabsVoicesError || "لإظهار أصواتك المخصصة: تأكد من ضبط ELEVENLABS_API_KEY في بيئة الخادم."}
                              </span>
                            </div>
                          )}

                          <details className="group">
                            <summary className="text-xs text-[#002845]/50 cursor-pointer hover:text-[#002845]/70 transition">
                              أصوات OpenAI الافتراضية ▾
                            </summary>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-2">
                              {[
                                { id: "onyx", name: "فيصل", desc: "أجهر (عميق)" },
                                { id: "ash", name: "عاصم", desc: "رجالي قوي" },
                                { id: "fable", name: "فابل", desc: "واثق" },
                                { id: "echo", name: "سعد", desc: "واضح" },
                                { id: "alloy", name: "خالد", desc: "محايد" },
                              ].map((v) => {
                                const isSelected = videoVoice === v.id;
                                return (
                                  <div
